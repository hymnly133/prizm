...
async function loadTraySettings(): Promise<void> {
	try {
		...
		console.warn(
			"[Electron] Failed to load tray settings, using defaults:",
			err
		);
		trayEnabled = true;
		minimizeToTray = true;
...
function createMainWindow(): BrowserWindow {
	...
	mainWindow.webContents.on("will-navigate", (event, url) => {
		const isAppUrl =
			url.startsWith("http://localhost:5183") ||
			url.startsWith("file://") ||
			url === "about:blank";
		if (!isAppUrl) {
			event.preventDefault();
...
function logNotify(...args: unknown[]) {
	if (DEBUG_NOTIFY) console.log("[Notify]", ...args);
}
...
/** 待发送通知队列（窗口未就绪时缓存） */
const notificationQueue: { title: string; body?: string; source?: string }[] =
	[];
...
function registerIpcHandlers(): void {
	...
	ipcMain.handle(
		...
		async (
			...
			try {
				const register = await registerClientOnServer(
					serverUrl,
					name,
					requestedScopes
				);
	...
	ipcMain.handle(
		"test_connection",
		async (_event, { serverUrl }: { serverUrl: string }) => {
			try {
	...
	ipcMain.handle(
		...
		async (
			_event,
			{
				...
				apiKey,
				scope,
			}: { serverUrl: string; apiKey?: string; scope?: string }
		) => {
	...
	ipcMain.handle(
		"show_notification",
		(_event, payload: { title: string; body?: string; source?: string }) => {
