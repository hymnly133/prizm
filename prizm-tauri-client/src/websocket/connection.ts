...
export class PrizmWebSocketClient {
  ...
  private eventHandlers = new Map<WebSocketEventType, Set<WebSocketEventHandler<WebSocketEventType>>>()
  ...
  async connect(): Promise<void> {
    const wsUrl = `ws://${this.config.host}:${this.config.port}/ws?apiKey=${encodeURIComponent(this.config.apiKey)}`
    ...
    return new Promise((resolve, reject) => {
      ...
      this.ws.onopen = () => {
        console.log('[PrizmClient] WebSocket connected')
        this.emit('connected', { type: 'connected', clientId: '', serverTime: Date.now() })

        // 自动注册通知事件
        this.registerEvent('notification')
      ...
      this.ws.onclose = (event: CloseEvent) => {
        console.log(`[PrizmClient] WebSocket closed: ${event.code} - ${event.reason}`)
  ...
  private handleMessage(data: string): void {
    try {
      ...
      switch (message.type) {
        case 'connected':
          console.log('[PrizmClient] Server acknowledged connection:', message.clientId)
          break

        case 'registered':
          console.log(`[PrizmClient] Registered for event: ${message.eventType}`)
          break

        case 'unregistered':
          console.log(`[PrizmClient] Unregistered from event: ${message.eventType}`)
          break
        ...
        case 'error':
          console.error(`[PrizmClient] Server error [${message.code}]: ${message.message}`)
          break
  ...
  private handleEventPush(message: EventPushMessage<NotificationPayload>): void {
    if (message.eventType === 'notification') {
      ...
      console.log(`[PrizmClient] Received notification: ${payload.title}`)
  ...
  private send(data: ClientMessage): void {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) {
      console.warn('[PrizmClient] WebSocket not connected, cannot send message')
      return
    }
  ...
  private emit<T extends WebSocketEventType>(eventType: T, data: WebSocketClientEventMap[T]): void {
    ...
    if (handlers) {
      for (const handler of handlers) {
        try {
          handler(data as WebSocketClientEventMap[T])
        } catch (error) {
