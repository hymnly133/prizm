...
import { importMcpConfigFromFile, discoverMcpConfigFiles } from '../mcp-client/mcpConfigImporter'
...
export function createMcpConfigRoutes(router: Router): void {
  ...
  // POST /mcp/import - 从外部 MCP 配置文件导入（Cursor/Claude Code/VS Code 格式）
  router.post('/mcp/import', (req: Request, res: Response) => {
    try {
      const { source, path: customPath } = req.body ?? {}
      if (!source || !['cursor', 'claude-code', 'vscode'].includes(source)) {
        return res
          .status(400)
          .json({ error: 'source must be "cursor", "claude-code", or "vscode"' })
      }

      let importPath: string
      if (typeof customPath === 'string' && customPath.trim()) {
        importPath = customPath.trim()
      } else {
        const cwd = process.cwd()
        if (source === 'cursor') {
          importPath = require('path').join(cwd, '.cursor', 'mcp.json')
        } else if (source === 'claude-code') {
          importPath = require('path').join(cwd, '.mcp.json')
        } else {
          importPath = require('path').join(cwd, '.vscode', 'mcp.json')
        }
      }

      const configs = importMcpConfigFromFile(importPath)
      const imported: McpServerConfig[] = []
      const skipped: string[] = []

      for (const config of configs) {
        try {
          const existing = getMcpServerById(config.id)
          if (existing) {
          ...
          log.warn('Failed to import MCP server %s:', config.id, err)
          skipped.push(config.id)
        }
      }

      res.json({ imported: imported.length, skipped, servers: imported })
    } catch (error) {
      log.error('import MCP config error:', error)
      const { status, body } = toErrorResponse(error)
      res.status(status).json(body)
    }
  })

  // GET /mcp/discover - 发现项目中的 MCP 配置文件
  router.get('/mcp/discover', (_req: Request, res: Response) => {
    try {
      const sources = discoverMcpConfigFiles()
      res.json({ sources })
    } catch (error) {
      log.error('discover MCP configs error:', error)
      const { status, body } = toErrorResponse(error)
      res.status(status).json(body)
    }
  })
