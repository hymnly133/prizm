...
export class EventRegistry {
  ...
  registerClient(context: WebSocketContext): void {
    ...
    log.info('Registered client', context.clientId, context.id)
  }
  ...
  unregisterClient(contextId: string): void {
    ...
    log.info('Unregistered client', context.clientId, contextId)
  }
  ...
  registerEvent(contextId: string, eventType: EventType): void {
    ...
    log.info('Client', contextId, 'registered for event', eventType)
  }
  ...
  unregisterEvent(contextId: string, eventType: EventType): void {
    ...
    log.info('Client', contextId, 'unregistered from event', eventType)
  }
  ...
  broadcastToClient(
    ...
    if (!context || !context.isOpen()) {
      log.warn('Client', clientId, 'not found or not connected')
      return false
    }

    // 检查 scope 权限
    if (scope && !context.hasScopePermission(scope)) {
      log.warn('Client', clientId, 'does not have permission for scope', scope)
      return false
    }
  ...
  cleanup(): void {
    ...
    if (toRemove.length > 0) {
      log.info('Cleaned up', toRemove.length, 'disconnected clients')
    }
  }
}
