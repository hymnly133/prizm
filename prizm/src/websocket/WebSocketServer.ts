...
import { Server as WSServer, type WebSocket } from 'ws'
...
export class WebSocketServer {
  ...
  constructor(
    ...
    console.log(`[Prizm WebSocket Server] Initialized on path ${this.options.path}`)
  }
  ...
  protected createEventRegistry(): EventRegistry {
    const { EventRegistry: ER } = require('./EventRegistry')
    return new ER()
  }
  ...
  private setupConnectionHandlers(): void {
    ...
    this.wss.on('error', (error) => {
      console.error('[Prizm WebSocket Server] Error:', error)
    })

    this.wss.on('close', () => {
      console.log('[Prizm WebSocket Server] Closed')
    })
  ...
  private async handleConnection(socket: WebSocket, req: http.IncomingMessage): Promise<void> {
    ...
    console.log(`[Prizm WebSocket Server] New connection ${connectionId}`)
    ...
    try {
      ...
      socket.on('error', (error) => {
        console.error(`[Prizm WebSocket Server] Error for ${connectionId}:`, error)
        this.eventRegistry.unregisterClient(connectionId)
      })

      console.log(`[Prizm WebSocket Server] Client ${clientId} authenticated (${connectionId})`)

    } catch (error) {
      console.error(`[Prizm WebSocket Server] Error handling connection ${connectionId}:`, error)
  ...
  protected createWebSocketContext(
    ...
    const { WebSocketContext: WSC } = require('./WebSocketContext')
    return new WSC(id, clientId, allowedScopes, socket)
  }
  ...
  private handleMessage(context: WebSocketContext, message: string): void {
    try {
      ...
      console.error(`[Prizm WebSocket Server] Failed to parse message from ${context.clientId}:`, error)
  ...
  private processMessage(context: WebSocketContext, message: ClientToServerMessage): void {
    switch (message.type) {
      ...
      default:
        this.sendError(context.socket, 'UNKNOWN_MESSAGE_TYPE', `Unknown message type: ${(message as { type: string }).type}`)
    }
  }
  ...
  private sendMessage(socket: WebSocket, message: ServerToClientMessage): boolean {
    ...
    try {
      ...
      console.error('[Prizm WebSocket Server] Failed to send message:', error)
      return false
    }
  }
   ...
   * 获取所有已连接的客户端
   */
  getConnectedClients(): Array<{ clientId: string; registeredEvents: EventType[]; currentScope: string }> {
   ...
   */
  destroy(): void {
    console.log('[Prizm WebSocket Server] Shutting down...')
    ...
    this.wss.close(() => {
      console.log('[Prizm WebSocket Server] Shut down complete')
    })
  }
}
