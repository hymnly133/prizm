...
export function createPrizmServer(
  ...
  const { port = 4127, host = '127.0.0.1', enableCors = true, authEnabled = true, enableWebSocket = true, websocketPath = '/ws' } = options
  ...
  app.use((req, _res, next) => {
    ;(req as any).prizmServer = wsServer
    next()
  })
  ...
  app.use(
    (err: Error, _req: express.Request, res: express.Response, _next: express.NextFunction) => {
      console.error('[Prizm Server] Error:', err)
  ...
  return {
    async start(): Promise<void> {
      if (isRunning) {
        console.warn('[Prizm Server] Server is already running')
        return
      }
        ...
        try {
          server = app.listen(port, host, () => {
            ...
            if (enableWebSocket && server) {
              wsServer = new WebSocketServer(server, clientRegistry, { path: websocketPath })
              console.log(`[Prizm Server] WebSocket: ws://${host}:${port}${websocketPath}`)
            }

            console.log(`[Prizm Server] Listening on http://${host}:${port}`)
            resolve()
          })

          server.on('error', (error) => {
            console.error('[Prizm Server] Server error:', error)
            reject(error)
          })
        } catch (error) {
          reject(error)
    ...
    async stop(): Promise<void> {
      ...
      return new Promise((resolve, reject) => {
        server!.close((error) => {
          if (error) {
            console.error('[Prizm Server] Error stopping server:', error)
            reject(error)
          } else {
            isRunning = false
            server = null
            console.log('[Prizm Server] Server stopped')
            resolve()
          }
        })
      })
