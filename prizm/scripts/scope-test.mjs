...
async function fetchWithScope(path, scope, method = "GET") {
	...
	if (!res.ok) {
		throw new Error(
			`${path}?scope=${scope} => ${res.status} ${await res.text()}`
		);
	}
	return res.json();
}
...
async function run() {
	...
	try {
		...
		console.error(
			"无法连接服务端，请确保已启动 (yarn dev 或 yarn start):",
			e.message
		);
		process.exit(1);
	}
	...
	if (!API_KEY) {
		try {
			...
			if (process.env.PRIZM_AUTH_DISABLED === "1") {
				...
				console.error(
					"需要 API Key。请设置 PRIZM_API_KEY 或 PRIZM_AUTH_DISABLED=1"
				);
				console.error(
					"或先启动服务后运行: PRIZM_AUTH_DISABLED=1 node scripts/scope-test.mjs"
				);
				process.exit(1);
			}
		}
