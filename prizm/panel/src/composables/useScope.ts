...
import { DEFAULT_SCOPE, ONLINE_SCOPE } from '@prizm/shared'
import { getScopes } from '../api/client'

const currentScope = ref(ONLINE_SCOPE)
const scopes = ref<string[]>([DEFAULT_SCOPE, ONLINE_SCOPE])
const scopeDescriptions = ref<Record<string, { label: string; description: string }>>({})

export function useScope() {
  async function loadScopes() {
    try {
      const res = await getScopes()
      scopes.value = res.scopes?.length ? res.scopes : [DEFAULT_SCOPE, ONLINE_SCOPE]
      ...
      if (!scopes.value.includes(currentScope.value)) {
        currentScope.value = res.scopes?.includes(ONLINE_SCOPE)
          ? ONLINE_SCOPE
          : scopes.value[0] ?? DEFAULT_SCOPE
      }
    } catch {
      scopes.value = [DEFAULT_SCOPE, ONLINE_SCOPE]
