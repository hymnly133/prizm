	...
	<div class="space-y-6">
		...
		<template v-else>
			<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
				...
				<div class="rounded-lg border border-zinc-700 bg-zinc-800/50 p-6">
					<h2 class="mb-2 text-sm font-medium text-zinc-400">任务</h2>
					<p class="text-lg font-semibold text-zinc-100">{{ tasksCount }}</p>
				</div>
				<div class="rounded-lg border border-zinc-700 bg-zinc-800/50 p-6">
...
async function load() {
	...
	try {
		...
		const [healthRes, notesRes, clientsRes, tasksRes, docsRes, sessionsRes] =
			await Promise.all([
				getHealth(),
				getNotes(scope),
				getClients(),
				getTasks(scope).catch(() => ({ tasks: [] })),
				getDocuments(scope).catch(() => ({ documents: [] })),
				listAgentSessions(scope).catch(() => ({ sessions: [] })),
			]);
		health.value = healthRes;
		notesCount.value = notesRes.notes?.length ?? 0;
		tasksCount.value = tasksRes.tasks?.length ?? 0;
