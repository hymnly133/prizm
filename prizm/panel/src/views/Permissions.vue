<template>
	<div class="space-y-6">
		...
		<div
			v-else-if="listError"
			class="rounded-lg border border-red-900/50 bg-red-950/30 p-6"
		>
			...
			<table class="w-full">
				<thead class="bg-zinc-800/80">
					<tr>
						...
						<th class="px-4 py-3 text-right text-sm font-medium text-zinc-400">
							操作
						</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-zinc-700">
					...
					>
						...
						<td class="px-4 py-3 font-mono text-sm text-zinc-400">
							{{ c.clientId }}
						</td>
						...
						<td class="px-4 py-3 text-right">
							<div class="flex justify-end gap-2">
								<button
									...
									:disabled="regenerating === c.clientId"
									@click="regenerateKey(c)"
								>
									{{ regenerating === c.clientId ? "生成中..." : "生成新 Key" }}
								</button>
								<button
									type="button"
				...
				>
					...
					<form class="space-y-4" @submit.prevent="doRegister">
						...
						>
							...
							<code class="mt-1 block break-all font-mono">{{
								newApiKey
							}}</code>
						</p>
						<div class="flex justify-end gap-2">
			...
			>
				...
				>
					<h2 class="mb-4 text-lg font-semibold">
						新 API Key（{{ regenerateTarget?.name }}）
					</h2>
					<p class="mb-3 text-sm text-amber-400">
						该 Key 仅展示一次，请妥善保存。旧 Key 已失效。
					</p>
					<code
						class="mb-4 block break-all rounded border border-emerald-700 bg-emerald-950/50 p-3 font-mono text-sm text-emerald-300"
					>
						{{ regeneratedKey }}
					</code>
...
const regenerateTarget = ref<ClientInfo | null>(null);
...
async function revokeClient(c: ClientInfo) {
	if (!confirm(`确定要吊销客户端「${c.name}」吗？该客户端将无法再访问 API。`))
		return;
...
async function regenerateKey(c: ClientInfo) {
	if (!confirm(`确定要为「${c.name}」重新生成 API Key 吗？旧 Key 将立即失效。`))
		return;
	regenerating.value = c.clientId;
	try {
		const res = await regenerateClientApiKey(c.clientId);
		regeneratedKey.value = res.apiKey;
		regenerateTarget.value = c;
